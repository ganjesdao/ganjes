...
// ============================================================================
// Total Supply: 10,000,000 GNJS (PERMANENTLY FIXED - NO MINTING POSSIBLE)
...
abstract contract MultiSigOwnable is Context {
    ...
    modifier notConfirmed(uint256 transactionId) {
        require(
            !confirmations[transactionId][_msgSender()],
            "MultiSig: transaction already confirmed"
        );
        _;
    }
        ...
        notExecuted(transactionId)
    {
        require(
            confirmations[transactionId][_msgSender()],
            "MultiSig: transaction not confirmed"
        );
        ...
        notExecuted(transactionId)
    {
        require(
            ...
            "MultiSig: cannot execute transaction"
        );
 ...
 *
 * KEY FEATURES:
 * - Fixed Supply: 10,000,000 GNJS tokens (CANNOT BE INCREASED)
    ...
    ReentrancyGuard
{
    ...
    uint256 public maxDailyTransferLimit = 1_000_000 * 10 ** _decimals; // 1M tokens per day
    mapping(address => uint256) public dailyTransferAmount;
    mapping(address => uint256) public lastTransferDay;
    
    // Emergency controls
    bool public emergencyMode = false;
    uint256 public constant EMERGENCY_TIMELOCK = 1 hours; // Reduced timelock for emergencies
    ...
    event BurnAction(address indexed from, uint256 value, string burnType);
    event EmergencyWithdrawal(address indexed recipient, uint256 amount);
    event SecurityAlert(string alertType, address indexed triggeredBy, uint256 timestamp);
    event BlacklistUpdated(address indexed account, bool status, address indexed updatedBy);
    event DailyLimitUpdated(uint256 oldLimit, uint256 newLimit, address indexed updatedBy);
    ...
    /**
     * @dev Internal pause function - only callable through multi-sig transactions
     * @notice This function can ONLY be called through executeTransaction()
     */
    function _pauseContract() external {
        require(_msgSender() == address(this), "MultiSig: only callable through governance");
        _pause();
        emit EmergencyAction("PAUSE", tx.origin);
    }

    /**
     * @dev Internal unpause function - only callable through multi-sig transactions
     * @notice This function can ONLY be called through executeTransaction()
     */
    function _unpauseContract() external {
        require(_msgSender() == address(this), "MultiSig: only callable through governance");
        _unpause();
        emit EmergencyAction("UNPAUSE", tx.origin);
    }

    /**
     * @dev Emergency pause - reduced timelock for critical situations
     * @notice Only callable through multi-sig with reduced 1-hour timelock
     */
    function _emergencyPauseContract() external {
        require(_msgSender() == address(this), "MultiSig: only callable through governance");
        _pause();
        emit EmergencyAction("EMERGENCY_PAUSE", tx.origin);
    }
     ...
     * @notice Only multi-sig owners can burn small amounts directly
     * @notice Large burns (>1% of total supply) require multi-sig approval
     */
    function burn(uint256 amount) external onlyOwner nonReentrant {
        uint256 maxDirectBurn = _totalSupply / 100; // 1% of total supply
        require(amount <= maxDirectBurn, "BEP20: large burns require multi-sig approval");
        _burn(_msgSender(), amount);
        emit BurnAction(_msgSender(), amount, "DIRECT_BURN");
    }

    /**
     * @dev Internal function for large burns - only callable through multi-sig
     * @param account Account to burn tokens from
     * @param amount Amount of tokens to burn
     */
    function _burnLarge(address account, uint256 amount) external nonReentrant {
        require(_msgSender() == address(this), "MultiSig: only callable through governance");
        require(isOwner[account], "MultiSig: can only burn from owner accounts");
        _burn(account, amount);
        emit BurnAction(account, amount, "MULTISIG_BURN");
    }
    ...
    function createPauseTransaction() external onlyOwner returns (uint256) {
        bytes memory data = abi.encodeWithSignature("_pauseContract()");
        return submitTransaction(address(this), data, "Multi-Sig: Pause token transfers");
    }
    ...
    function createUnpauseTransaction() external onlyOwner returns (uint256) {
        bytes memory data = abi.encodeWithSignature("_unpauseContract()");
        return submitTransaction(address(this), data, "Multi-Sig: Unpause token transfers");
    }

    /**
     * @dev Helper function to create emergency pause transaction
     * @return transactionId ID of the created transaction
     */
    function createEmergencyPauseTransaction() external onlyOwner returns (uint256) {
        bytes memory data = abi.encodeWithSignature("_emergencyPauseContract()");
        return submitTransaction(address(this), data, "Multi-Sig: EMERGENCY pause token transfers");
    }

    /**
     * @dev Helper function to create emergency withdrawal transaction
     * @param recipient Address to receive withdrawn tokens (must be an owner)
     * @return transactionId ID of the created transaction
     */
    function createEmergencyWithdrawTransaction(address recipient) external onlyOwner returns (uint256) {
        require(isOwner[recipient], "MultiSig: recipient must be an owner");
        bytes memory data = abi.encodeWithSignature("_emergencyWithdraw(address)", recipient);
        return submitTransaction(address(this), data, "Multi-Sig: Emergency token withdrawal");
    }

    /**
     * @dev Helper function to create large burn transaction
     * @param account Owner account to burn tokens from
     * @param amount Amount of tokens to burn
     * @return transactionId ID of the created transaction
     */
    function createLargeBurnTransaction(address account, uint256 amount) external onlyOwner returns (uint256) {
        require(isOwner[account], "MultiSig: can only burn from owner accounts");
        require(amount > _totalSupply / 100, "BEP20: use direct burn() for small amounts");
        bytes memory data = abi.encodeWithSignature("_burnLarge(address,uint256)", account, amount);
        return submitTransaction(address(this), data, "Multi-Sig: Large token burn operation");
    }
    ...
    function _transfer(address from, address to, uint256 amount) internal {
        ...
        require(!blacklisted[from], "BEP20: sender is blacklisted");
        require(!blacklisted[to], "BEP20: recipient is blacklisted");
        
        // Enhanced validation for large transfers
        if (amount > _totalSupply / 10) { // Transfers > 10% of supply
            emit SecurityAlert("LARGE_TRANSFER", from, block.timestamp);
        }
        
        // Daily transfer limit check (except for owners and contract)
        if (!isOwner[from] && from != address(this)) {
            ...
            require(
                dailyTransferAmount[from] + amount <= maxDailyTransferLimit,
                "BEP20: daily transfer limit exceeded"
            );
    ...
    /**
     * @dev Emergency withdrawal - only callable through multi-sig governance
     * @notice Requires multi-sig approval + timelock for security
     * @param recipient Address to receive the withdrawn tokens
     */
    function _emergencyWithdraw(address recipient) external {
        require(_msgSender() == address(this), "MultiSig: only callable through governance");
        require(recipient != address(0), "BEP20: invalid recipient");
        require(isOwner[recipient], "MultiSig: recipient must be an owner");
        
        uint256 balance = _balances[address(this)];
        require(balance > 0, "BEP20: no tokens to withdraw");
        
        _transfer(address(this), recipient, balance);
        emit EmergencyAction("EMERGENCY_WITHDRAW", recipient);
    ...
    function getContractInfo() external pure returns (string memory) {
        return
            "Ganjes Token (GNJS) - Multi-Sig Governed, Fixed Supply: 10M, Pausable, Burnable, Secure";
    }
    ...
    // ============================================================================
    // ENHANCED SECURITY FUNCTIONS (MULTI-SIG ONLY)
    // ============================================================================
    
    /**
     * @dev Blacklist/unblacklist an address - only through multi-sig
     * @param account Address to blacklist/unblacklist
     * @param status True to blacklist, false to unblacklist
     */
    function _setBlacklisted(address account, bool status) external {
        require(_msgSender() == address(this), "MultiSig: only callable through governance");
        require(account != address(0), "BEP20: invalid address");
        require(!isOwner[account], "MultiSig: cannot blacklist owners");
        
        blacklisted[account] = status;
        emit BlacklistUpdated(account, status, tx.origin);
        emit SecurityAlert(status ? "BLACKLIST_ADD" : "BLACKLIST_REMOVE", account, block.timestamp);
    }
    
    /**
     * @dev Update daily transfer limit - only through multi-sig
     * @param newLimit New daily transfer limit
     */
    function _setDailyTransferLimit(uint256 newLimit) external {
        require(_msgSender() == address(this), "MultiSig: only callable through governance");
        require(newLimit > 0, "BEP20: invalid limit");
        
        uint256 oldLimit = maxDailyTransferLimit;
        maxDailyTransferLimit = newLimit;
        emit DailyLimitUpdated(oldLimit, newLimit, tx.origin);
        emit SecurityAlert("DAILY_LIMIT_UPDATED", address(0), block.timestamp);
    }
    
    /**
     * @dev Helper to create blacklist transaction
     */
    function createBlacklistTransaction(address account, bool status) external onlyOwner returns (uint256) {
        require(account != address(0), "BEP20: invalid address");
        require(!isOwner[account], "MultiSig: cannot blacklist owners");
        
        bytes memory data = abi.encodeWithSignature("_setBlacklisted(address,bool)", account, status);
        string memory description = status ? "Multi-Sig: Blacklist address" : "Multi-Sig: Remove address from blacklist";
        return submitTransaction(address(this), data, description);
    }
    
    /**
     * @dev Helper to create daily limit update transaction
     */
    function createDailyLimitTransaction(uint256 newLimit) external onlyOwner returns (uint256) {
        require(newLimit > 0, "BEP20: invalid limit");
        
        bytes memory data = abi.encodeWithSignature("_setDailyTransferLimit(uint256)", newLimit);
        return submitTransaction(address(this), data, "Multi-Sig: Update daily transfer limit");
    }
}
