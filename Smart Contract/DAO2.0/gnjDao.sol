...
contract GanjesDAO {
    ...
    mapping(uint256 => uint256) public proposalFees; // proposalId => fee amount
    mapping(address => uint256) public totalProposalFeesPaid; // total fees paid by proposer
    ...
    event ProposalFeeRefunded(uint256 indexed proposalId, address indexed proposer, uint256 amount, uint256 blockNumber, uint256 timestamp);
    event ProposalFeeCollected(uint256 indexed proposalId, address indexed proposer, uint256 amount, uint256 blockNumber, uint256 timestamp);
    ...
    // Internal function to refund proposal fee
    ...
    // Refund proposal fee for failed proposals (admin function)
    ...
    // Proposal fee utility functions
    ...
    function isProposalFeeRefunded(uint256 _proposalId) external view returns (bool) {
        require(_proposalId > 0 && _proposalId <= proposalCount, "Invalid proposal ID");
        return proposalFees[_proposalId] == 0;
    }
